...
## I/O Multiplexing Threadpool
  ...
  - 이에 비해 I/O Multiplexing은 한 스레드 안에서 여러 소켓을 다룰 수 있다. 
    ...
    - 예전에 게임 서버를 개발할 때는 리스닝 스레드(main thread)에서 accept 루프를 돌면서 클라이언트 연결을 게임 로직 스레드풀과 네트워크 전담 스레드로 넘겼다. 게임 로직 스레드풀의 각 스레드는 epoll 객체에 클라이언트 소켓을 등록한 뒤 클라이언트에서 데이터가 오면 로직을 실행한 후 처리 결과를 네트워크 전담 스레드의 처리 결과 큐(processed result queue)에 넣어두었다. 네트워크 전담 스레드는 일정 시간(1초)마다 처리 결과 큐에서 클라이언트가 보낸 요청에 대한 결과(예를 들어 적을 공격한 후 내 체력과 적의 체력, 죽음 판정, 남은 총알 갯수 등)를 꺼내 모든 클라이언트에 브로드캐스팅하도록 구현했다. 
